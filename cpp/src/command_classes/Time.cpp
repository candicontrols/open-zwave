...
//	Time.cpp
//
//	Implementation of the Z-Wave COMMAND_CLASS_TIME
//
...
static char const* c_monthNames[] = 
{
	"Invalid",
	"January",
	"Feburary",
...
{
	Msg* msg = new Msg( "TimeCmd_Get", GetNodeId(), REQUEST, FUNC_ID_ZW_SEND_DATA, true, true, FUNC_ID_APPLICATION_COMMAND_HANDLER, GetCommandClassId() );
	msg->SetInstance( this, _instance );
	msg->Append( GetNodeId() );
	msg->Append( 2 );
	msg->Append( GetCommandClassId() );
	msg->Append( TimeCmd_Get );
...
{
	if (TimeCmd_Report == (TimeCmd)_data[0])
	{
    /*
		uint8 day = _data[1] >> 5;
      ...
      Log::Write( LogLevel_Info, GetNodeId(), "Received Time report: %d %d %d", _data[1], _data[2], _data[3] );
...
)
{
	if( Node* node = GetNodeUnsafe() )
	{
		...
		{	
			ValueList::Item item;
			item.m_label = c_monthNames[i];
			item.m_value = i;
			items.push_back( item ); 
		}

		node->CreateValueInt( ValueID::ValueGenre_User, GetCommandClassId(), _instance, TimeIndex_Year, "Year", "", false, false, 0, 0 );
    node->CreateValueList( ValueID::ValueGenre_User, GetCommandClassId(), _instance, TimeIndex_Month, "Month", "", false, false, 1, items, 0, 0 );
		node->CreateValueByte( ValueID::ValueGenre_User, GetCommandClassId(), _instance, TimeIndex_Day, "Day", "", false, false, 0, 0 );
	}
}
